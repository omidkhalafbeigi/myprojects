...
#dar in barname, anvare voroodi ba pattern haye motefavet be sorate pishfarz gharar dade shode.
...
##---------------------------------------------------------------------------------------
def HFCM_ridge(dataset1, ratio=0.7, plot_flag=False): #Function baraye predict data
    #Function khodesh behtarin alpha ro entekhab mikone...
...
def analyze_parameter():
    ...
    # data set : sunspot
    sunspot = pd.read_csv('C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/sunspot.csv', delimiter=';').as_matrix()
    dataset = sunspot[:-1, 1]
    ratio = 0.7674
    ...
    sp500_src = "C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/sp500.csv"
    ...
    writer = pd.ExcelWriter('D:/output_Z047.xlsx')
    ...
    import shutil
    import os
    if not os.path.exists('D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Nc'):
        os.makedirs('D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Nc')

    if not os.path.exists('D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Order'):
        os.makedirs('D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Order')

    for order in Order_list:
        df = pd.DataFrame({r'$N_c$': Nc_list,
                           'Z047 time series': df1[order].values})
        df = pd.melt(df, id_vars=r'$N_c$', var_name="Dataset", value_name='RMSE')
        g = sns.catplot(x=r'$N_c$', y='RMSE', hue='Dataset',
                           hue_order=['Z047 time series', 'S&P500'], data=df, kind='bar',
        ...
        plt.savefig(
            r"D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Nc/k=%d.pdf" % order)
        plt.savefig(
            r"D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Nc/k=%d.png" % order)
        plt.close()

    for Nc in Nc_list:
        # / print(len(df_1.loc[Nc, :]))
        df = pd.DataFrame({'$k$': Order_list,
                           'Z047 time series': df1.loc[Nc, :].values})
        ...
        plt.savefig(
            r"D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Order/Nc=%d.pdf" % Nc)
        plt.savefig(
            r"D:/WHFCM/Outcome_for_papers/impact_parameters/varying_Order/Nc=%d.png" % Nc)
        plt.close()
...
def main():
    ...
    #sunspot = pd.read_csv(r'C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/monthly-closings-of-the-dowjones.csv', delimiter=',', parse_dates=[0],
    ...
    #dateparse = lambda x: pd.datetime.strptime(x, '%Y-%m')  # %Y-%m-%d
    #sunspot = pd.read_csv(r'C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/monthly-milk-production-pounds-p.csv', delimiter=',', parse_dates=[0],
    #                      date_parser=dateparse).as_matrix()
    ...
    #dateparse = lambda x: pd.datetime.strptime(x, '%Y-%m')  # %Y-%m-%d
    #sunspot = pd.read_csv(r'C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/monthly-critical-radio-frequenci.csv', delimiter=',', parse_dates=[0],
    #                      date_parser=dateparse).as_matrix()
    #dataset = sunspot[:, 1].astype(np.float)
    #time = sunspot[:, 0]
    #ratio = 0.75
    ...
    #--------
    #dateparse = lambda x: pd.datetime.strptime(x, '%Y-%m')  # %Y-%m-%d
    #sunspot = pd.read_csv(r'C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/co2-ppm-mauna-loa-19651980.csv', delimiter=',', parse_dates=[0],
    #                      date_parser=dateparse).as_matrix()
    #dataset = sunspot[:, 1].astype(np.float)
    #time = sunspot[:, 0]
    #ratio = 0.85
    ...
    #dateparse = lambda x: pd.datetime.strptime(x, '%Y-%m')  # %Y-%m-%d
    #sunspot = pd.read_csv(r'C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/monthly-lake-erie-levels-1921-19.csv', delimiter=',', parse_dates=[0],
    #                      date_parser=dateparse).as_matrix()
    #dataset = sunspot[:, 1].astype(np.float)
    #time = sunspot[:, 0]
    #ratio = 0.7674
    ...
    #D:/Z047.csv
    # data set 1: sunspot
    sunspot = pd.read_csv('D:/Z047.csv', delimiter=';').as_matrix()
    dataset = sunspot[:-1, 0]
    np.savetxt('D:/Original.txt', dataset, delimiter=',')
    time = sunspot[:-1, 1]
    ratio = 0.7674
    #---------------------------------
    #
    ...
    import scipy.io as sio
    #dataset = sio.loadmat('C:/Users/pq/Downloads/Compressed/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/MG_chaos.mat')['dataset'].flatten()
    # only use data from t=124 : t=1123  (all data previous are not in the same pattern!)
    #dataset = pd.read_csv('D:/Z001 (3).csv')
    #dataset = dataset[123:1123]
    #time = range(len(dataset))
    ##time = range(5,23500,5)
    #ratio = 0.5
    ...
    np.savetxt("D:/Predicted.txt", data_predicted)
    np.savetxt("D:/Error File.txt", dataset - data_predicted)
    # Outcomes
    ...
    error_data = dataset - data_predicted
    mytime = 5.7
    sum = 0
    for i in error_data:
        point = abs(float(i))
        sum += point
        ax41.plot(mytime, point, 'ro--')
        mytime += 5.7
    print("Average is: ", str(sum / 1882))
    #ax41.plot(time, dataset - data_predicted, 'co--', label="error")
    ...
    ax41.legend()
    plt.tight_layout()
    plt.show() #Rasm nemoodar marboot be maghadir predict shode va original
...
def statistics(origin, predicted):
    # # compute RMSE (Mizan khataye prediction)
    #error_file = open("D:/Error File.txt", mode="w")
    from sklearn.metrics import mean_squared_error
    mse = mean_squared_error(origin, predicted)
    #error_file.writelines(str(origin - predicted))
    #error_file.close()
    rmse = np.sqrt(mse)
...
#Handmade function for (sigma) (x - x^) / n
def Average_Handmade(File_Location):
    myfile = open(File_Location)
    taken_list = myfile.readlines()
    myfile.close()
    mysum = 0
    for item in taken_list:
        mysum += abs(float(item))
    myaverage = (mysum / len(taken_list))
    return myaverage


def GetAbs(File_Location2):
    myabs = open(File_Location2)
    taken2 = myabs.readlines()
    for i in taken2:
        print(abs(float(i)))
    myabs.close()

def Plot_Error_File(File_Location):
    myfile = open(File_Location)
    taken_list = myfile.readlines()
    myfile.close()
    #time = pd.read_csv('C:/Users/pq/Desktop/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/sunspot.csv', delimiter=';').as_matrix()[:-1, 0]
    for i in taken_list:
        plt.plot(float(i))
    plt.show()
...
if __name__ == '__main__':
    ...
    # main function
    main()
    #Plot_Error_File("D:/Sunspot/Error File.txt")
    #print("Average: " + str(Average_Handmade("D:/Error File.txt")))
    #GetAbs("D:/Error File.txt")