...
def analyze_parameter():
    ...
    # data set : sunspot
    ### sunspot = pd.read_csv('/media/omid/Multimedia/Projects/Wavelet HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/sunspot.csv', delimiter=',').as_matrix()
    # dataset = sunspot[:-1, 1]
    ### dataset = sunspot[1]
    ### ratio = 0.7674
    ...
    sp500_src = "/media/omid/Multimedia/Projects/Wavelet " \
                "HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/newfile_covid.csv"
    dateparse = lambda x: pd.datetime.strptime(x, '%m-%d-%Y')
    sp500 = pd.read_csv(sp500_src, delimiter=',', parse_dates=[1], date_parser=dateparse).as_matrix()
    dataset = np.array(sp500[:, 0], dtype=np.float)
    df2 = analyze_paras_HFCM(dataset, ratio=0.6)
    # save df1 & df2 to excel
    writer = pd.ExcelWriter('/media/omid/Multimedia/Projects/Wavelet '
                            'HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/output_covid19.xlsx')
...
def main():
    ...
    # --------
    # dateparse = lambda x: pd.datetime.strptime(x, '%Y-%m')  # %Y-%m-%d
    # sunspot = pd.read_csv(r'/media/omid/Multimedia/Projects/Wavelet HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/co2-ppm-mauna-loa-19651980.csv', delimiter=',', parse_dates=[0],
    #                       date_parser=dateparse).as_matrix()
    # dataset = sunspot[:, 1].astype(np.float)
    # time = sunspot[:, 0]
    # ratio = 0.85
    ...
    # data set 1: sunspot
    # sunspot = pd.read_csv('/media/omid/Multimedia/Projects/Wavelet HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/covid_19_data.csv', delimiter=';').as_matrix()
    # dataset = sunspot[:-1, 1]
    # dataset = sunspot[6]
    ...
    sp500_src = r"/media/omid/Multimedia/Projects/Wavelet HFCM/Wavelet-HFCM-master/Wavelet-HFCM-master/datasets/newfile_covid.csv"
    dateparse = lambda x: pd.datetime.strptime(x, '%m-%d-%Y')
    sp500 = pd.read_csv(sp500_src, delimiter=',', parse_dates=[1], date_parser=dateparse).as_matrix()
    dataset = np.array(sp500[0], dtype=np.float)
    time = sp500[1]
    ...
    mse, rmse, nmse = statistics(dataset[:len_train_data - len_validation_data],
                                 data_predicted[:len_train_data - len_validation_data])
    print('Forecasting on train dataset: MSE|RMSE|NMSE is : |%f |%f |%f|' % (np.power(rmse, 2), rmse, nmse))

    # Error of Validation dataset
    mse, rmse, nmse = statistics(dataset[len_train_data - len_validation_data:len_train_data],
                                 data_predicted[len_train_data - len_validation_data:len_train_data])
